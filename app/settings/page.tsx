...
] as const; // استفاده از as const برای استنتاج دقیق‌تر (مقادیر به‌عنوان literal شناخته می‌شوند)

// گزینه‌های تم با as const و نوع صریح
const themeOptions = [
...
] as const; // این باعث می‌شود value از نوع literal "light" | "dark" | "system" باشد

// استخراج نوع literal برای theme از روی آرایه (اختیاری، اما برای تایپ‌سیفتی بیشتر)
type ThemeValue = typeof themeOptions[number]["value"]; // "light" | "dark" | "system"

export default function SettingsPage() {
  const [settings, setSettings] = useState<SettingsState>(defaultSettings);
  const [loading, setLoading] = useState(true);

  // خواندن تنظیمات از IndexedDB
  useEffect(() => {
    let mounted = true;
    (async () => {
      try {
  ...
  const updateSettings = (patch: Partial<SettingsState>) => {
    setSettings((current) => {
      ...
      saveStoredSettings(next).catch((error) => {
  ...
  return (
    <div className="space-y-5 pb-24">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <p className="mt-1 max-w-xl text-sm leading-6 text-muted-foreground">
              اندازه فونت و حالت نمایش را به سبک دلخواه خود تنظیم کنید تا مطالعه
              شعر برایتان راحت‌تر شود.
            </p>
          </div>
      ...
      <Card className="border-border/70 shadow-sm">
        ...
        <CardContent className="space-y-3">
          <div className="rounded-2xl border border-border/70 bg-background/70 p-4">
            ...
            >
              «أحبُّكِ حُبًّا يَجعلُ الليلَ في العينِ مِصباحًا، ويفتحُ في القلبِ
              بابَ الرَّوْحِ.»
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
            <span className="rounded-full bg-accent px-2.5 py-1">
              حجم فعلی: {settings.fontSize}
            </span>
            <span className="rounded-full bg-accent px-2.5 py-1">
      ...
      <div className="flex flex-col gap-3 rounded-2xl border border-border/70 bg-background/70 p-4 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-sm text-muted-foreground">
          این تنظیمات روی این دستگاه ذخیره می‌شود و در بازدیدهای بعدی دوباره
          استفاده می‌شود.
        </p>
        <Button asChild>
